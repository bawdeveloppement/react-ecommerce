...
const PreviewCollection = ({ title, items, restrict, routeName }) => {
    ...
    <div className="collection-preview">
        <h1 className={`title`}>{isActive ? <><Link to="/shop" className="title-link inactive">Shop</Link> / </> : ""} { !isActive ? <Link to={routeName} className={`${isActive ? 'active' : ''} title-link`}>{title}</Link> : title }</h1>
